...
- **Coordinated wave bump.** Floors raised across the umbrella to
  pull every package up to the 1.2.2 wave:
  - `z4j-brain >= 1.2.2`
  - `z4j-core >= 1.2.2`, `z4j-bare >= 1.2.2`
  - `z4j-django >= 1.2.2`, `z4j-flask >= 1.2.2`, `z4j-fastapi >= 1.2.2`

### Added

- See the per-package CHANGELOGs for the full list. The 1.2.2 wave
  ships:
  - `projects.default_scheduler_owner` (brain) — per-project default
    scheduler the dashboard + reconcilers fall back to.
  - Declarative reconcilers in `z4j-django`, `z4j-flask`, and
    `z4j-fastapi` driven by shared `z4j_bare.declarative` logic and
    the new `z4j_core.celerybeat_compat` translator.
  - Audit-log retention sweeper, SQLite WAL checkpoint task, and
    self-watch Prometheus metrics in the brain.
  - `z4j upgrade` CLI for zero-friction package updates.
  - Dashboard surfacing of the per-row `scheduler` column.
