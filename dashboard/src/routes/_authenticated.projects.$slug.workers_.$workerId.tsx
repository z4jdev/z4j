 ...
 *
 * Data comes from z4j's /workers/{id} endpoint which
...
function WorkerDetailPage() {
  ...
  const agentId =
    agents?.find((a) => a.state === "online")?.id ?? agents?.[0]?.id;
  ...
  async function onPoolResize(delta: number) {
    ...
    try {
      ...
      toast.success(
        `Pool resize (${delta > 0 ? "+" : ""}${delta}) command issued`,
      );
    } catch (err) {
      const message =
    ...
    <>
      <PageShell>
        ...
        <PageHeader
          title={worker.name}
          icon={Server}
          description="Worker health and process details."
          badges={
            <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
              ...
              {worker.last_heartbeat && (
                <span>
                  Last heartbeat: {formatRelative(worker.last_heartbeat)}
                </span>
              )}
            </div>
          }
              ...
              <CardContent>
                <KVTable
                  rows={[
                    ...
                    [
                      "Max concurrency",
                      str(pool["max-concurrency"] ?? worker.concurrency),
                    ],
                    [
                      "Processes",
                      Array.isArray(pool.processes)
                        ? (pool.processes as number[]).join(", ")
                        : str(pool.processes),
                    ],
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead sortKey="c1">Routing key</TableHead>
                        <TableHead sortKey="c2">Durable</TableHead>
                        <TableHead sortKey="c3">Exclusive</TableHead>
          ...
          <TabsContent value="tasks">
            <div className="space-y-4">
              ...
              {Object.keys(total).length > 0 && (
                <Card className="overflow-hidden">
                  <CardHeader>
                    <CardTitle className="text-sm">Processed tasks</CardTitle>
                  </CardHeader>
                  <CardContent className="p-0">
                    <Table>
                      <TableHeader>
                        <TableRow>
                          <TableHead sortKey="c0">Task</TableHead>
                          <TableHead sortKey="c1" className="text-right">
                            Count
                          </TableHead>
                        </TableRow>
                      </TableHeader>
                ...
                <Card className="overflow-hidden">
                  ...
                  <CardContent className="p-0">
                    <Table>
                      <TableHeader>
                        <TableRow>
                          <TableHead sortKey="c0">Task name</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {[...registered].sort().map((name) => (
                          <TableRow sortValues={{ c0: name }} key={name}>
                            <TableCell className="font-mono text-sm">
                              {name}
                            </TableCell>
                          </TableRow>
        ...
        <DialogContent>
          <DialogHeader>
            ...
            {worker?.engine === "celery" ? (
              <DialogDescription>
                This broadcasts Celery's{" "}
                <span className="font-mono">pool_restart</span> to the worker.
                Child processes finish their in-flight tasks, then are respawned
                with fresh code - zero task loss. Continue?
              </DialogDescription>
            ) : (
              <DialogDescription>
                <span className="font-semibold text-warning">Heads-up:</span>{" "}
                <span className="font-mono">
                  {worker?.engine ?? "this engine"}
                </span>{" "}
                has no graceful pool-restart primitive (only Celery does). The
                agent will emit a{" "}
                <span className="font-mono">worker.offline</span> event, then
                call <span className="font-mono">os._exit(0)</span> - your host
                process supervisor (docker / k8s / systemd / supervisor)
                respawns it per its restart policy.
                <br />
                <br />
                <span className="font-semibold">
                  In-flight tasks will be killed
                </span>{" "}
                and re-delivered by the broker. Only proceed if your tasks are
                safe to run twice (idempotent).
                <br />
                <br />
                If the worker isn't running under a supervisor, the restart will
                be refused with a clean error - set{" "}
                <span className="font-mono">Z4J_ORCHESTRATED=1</span> to opt in
                explicitly.
              </DialogDescription>
            )}
          </DialogHeader>
