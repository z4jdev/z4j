...
interface LintDoc {
  ...
  workers: { engine: string; evaluated: boolean; findings: { rule_id: string; severity: string }[] }[];
}
...
const scheduleDocs = import.meta.glob<{ default: { items: ScheduleRecord[] } | ScheduleRecord[] }>(
  "../../src/lib/demo-data/projects/*/schedules.json",
  { eager: true },
);
...
describe("demo schedule-runs fixtures", () => {
  ...
  for (const [path, mod] of Object.entries(runsDocs)) {
    ...
    const schedulesMod = Object.entries(scheduleDocs).find(([p]) => projectOf(p) === project)?.[1];
    ...
    it(`${project}: every fire status is one the brain writes`, () => {
      for (const row of doc.items) {
        for (const c of row.runs) expect(known, `${row.schedule_id} ${c.status}`).toContain(c.status);
      }
    });
    ...
    it(`${project}: consecutive_failures matches the fixture's own history`, () => {
      ...
      for (const s of schedules) {
        ...
        expect(s.consecutive_failures, s.name).toBe(Math.min(run, doc.circuit_breaker_threshold));
      }
    });
    ...
    it(`${project}: failed cells carry error detail, in-flight cells carry none`, () => {
      for (const row of doc.items) {
        for (const c of row.runs) {
          if (c.status === "acked_failed" || c.status === "failed") expect(c.error_code, row.schedule_id).toBeTruthy();
    ...
    it(`${project}: an enabled, unpaused schedule is not seeded overdue`, () => {
      for (const s of schedules) {
        ...
        if (typeof s.next_run_at_offset_s === "number" && s.name !== "emails.send_scheduled_campaigns") {
  ...
  for (const [path, mod] of Object.entries(lintDocs)) {
    ...
    it(`${project}: only celery workers are evaluated and the counts agree`, () => {
      for (const w of doc.workers) {
        ...
        for (const f of w.findings) expect(f.rule_id).toMatch(/^celery\.[a-z-]+$/);
      }
      expect(doc.workers_evaluated).toBe(doc.workers.filter((w) => w.evaluated).length);
      expect(doc.workers_not_evaluated).toBe(doc.workers.filter((w) => !w.evaluated).length);
      const bySeverity: Record<string, number> = {};
      for (const w of doc.workers) for (const f of w.findings) bySeverity[f.severity] = (bySeverity[f.severity] ?? 0) + 1;
