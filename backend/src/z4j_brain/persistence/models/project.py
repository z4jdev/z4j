...
project, a Flask service, etc. Tokens are project-scoped and
dashboard navigation is project-scoped.
"""
...
class Project(PKMixin, TimestampsMixin, Base):
    ...
    #: Per-project automation kill switch. When False, the rule engine
    #: loads NO rules for this project, so no automation fires regardless
    #: of individual rule ``is_enabled`` state -- a single ADMIN-flippable
    #: off switch for the whole project's automation blast radius.
