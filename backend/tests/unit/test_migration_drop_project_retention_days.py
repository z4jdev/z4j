...
def _column_names(sync_conn) -> set[str]:
    return {col["name"] for col in inspect(sync_conn).get_columns("projects")}
...
async def test_round_trip_drop_and_recreate() -> None:
    ...
    try:
        ...
        # Model field removed -> create_all does NOT build the column.
        ...
        async with engine.connect() as conn:
            ...
            assert cols["retention_days"]["nullable"] is True
            assert "30" in str(cols["retention_days"]["default"])
